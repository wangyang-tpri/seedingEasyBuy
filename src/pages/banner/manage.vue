  ...
  <view class="manage-page">
    <view v-if="list.length > 0" class="list-wrap">
      <view
        ...
        :key="item.id"
        @click="
          item.linkType === 1 && item.linkValue && goDetail(item.linkValue)
        "
      >
        <view class="card-top">
          <image
          ...
          <view class="card-info">
            ...
            <text class="banner-link" v-if="item.linkType === 1">{{
      ...
      <view class="popup">
        ...
        <view class="form-item" @click="openProductPicker">
          ...
          <view class="form-right">
            <text :class="{ placeholder: !selectedProductName }">{{
              selectedProductName || "不关联"
            }}</text>
      ...
      <view class="popup">
        ...
        <scroll-view scroll-y class="product-scroll">
          <view
            ...
            :key="p.id"
            :class="['product-item', { active: form.linkValue == p.id }]"
            @click="selectProduct(p)"
          >
            <text>{{ p.name }}</text>
            <u-icon
              v-if="form.linkValue == p.id"
              name="checkbox-mark"
              color="#07C160"
              size="18"
            />
          </view>
          <view v-if="myProducts.length === 0" class="empty-hint"
            >暂无已上架商品</view
          >
        </scroll-view>
      </view>
  ...
  methods: {
    ...
    async loadMyProducts() {
      try {
        ...
        this.myProducts.forEach((p) => {
          this.productNames[String(p.id)] = p.name;
        });
      } catch (e) {}
    },
    ...
    async onSubmit() {
      ...
      try {
        let imageUrl = "";
        if (this.tempImage) {
          if (this.tempImage.indexOf("/api/") !== -1) {
            const idx = this.tempImage.indexOf("/api/");
            imageUrl = this.tempImage.substring(idx);
          } else {
            imageUrl = await this.uploadFile(this.tempImage);
          }
        }
        if (this.editingId) {
          await put("/banner/update", {
            id: this.editingId,
            title: this.form.title,
            imageUrl,
            linkType: this.form.linkType,
            linkValue: this.form.linkValue,
          });
        } else {
          await post("/banner/add", {
            title: this.form.title,
            imageUrl: imageUrl || "",
            linkType: this.form.linkType,
            linkValue: this.form.linkValue,
    ...
    async toggleStatus(item) {
      try {
        await post(`/banner/status/${item.id}`);
        this.loadList();
      } catch (e) {}
    },
    goDetail(id) {
...
.card-actions {
  ...
  margin-top: 16rpx;
  padding-top: 16rpx;
  border-top: 1rpx solid $bg-stripe;
}
.btn-edit {
  padding: 10rpx 32rpx;
  border-radius: 30rpx;
